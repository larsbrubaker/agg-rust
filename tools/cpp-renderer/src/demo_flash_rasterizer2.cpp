...
static void flash_palette2(agg::rgba8* colors) {
    ...
    for (int i = 0; i < 100; ++i) {
        ...
        // (r, g, b) in that order. C++ leaves argument evaluation order
        // unspecified (MSVC evaluates right-to-left), so constructing
        // rgba8(rand(), rand(), rand(), 230) directly would assign the first
        // byte to blue. Assigning in logical r,g,b order matches the example's
        // intent and the Rust port.
