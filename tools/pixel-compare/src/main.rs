...
fn cmd_verify(args: &[String]) {
    ...
    let rust_path = out_dir.join(format!("{}_rust_{}x{}.bmp", demo, width, height));
    save_image(&rust_path, &rust_buf).expect("Failed to save Rust output");
    println!("  Saved Rust output: {}", rust_path.display());

    // 2. Run C++ renderer
    let cpp_raw_path = out_dir.join(format!("{}_cpp_{}x{}.raw", demo, width, height));
    ...
    let cpp_bmp_path = out_dir.join(format!("{}_cpp_{}x{}.bmp", demo, width, height));
    ...
    if !result.identical {
        ...
        let sbs_path = out_dir.join(format!("{}_sidebyside_{}x{}.bmp", demo, width, height));
