...
///
/// IMPORTANT — this deliberately matches the comparison harness, which differs
/// from the literal upstream example:
///
/// * The upstream example (`cpp-references/agg-src/examples/compositing2.cpp:66`)
///   templates `generate_color_ramp` on the argument type, so `ColorT = agg::rgba`
///   and the stops are interpolated with the *double-precision* `rgba::gradient`,
///   converting to `rgba8` only on assignment into the `pod_auto_array<rgba8>`.
/// * The headless harness that produces our ground-truth reference
///   (`tools/cpp-renderer/src/demo_compositing2.cpp`) instead pre-converts the
///   stops to `rgba8` (`color a1(c1); ...`) and interpolates with the
///   *fixed-point* `rgba8::gradient` (`lerp`, `ik = uround(k * 255)`).
///
/// These two paths coincide at `alpha == 1.0` (0 differing ramp entries) but
/// diverge at partial alpha: e.g. at `alpha = 0.5`, 10 segment-3 entries differ by
/// 1 in the alpha channel, producing 12260 differing output bytes. Since the
/// reference is generated by the harness, we reproduce the harness's fixed-point
/// path here (verified byte-identical at both `alpha = 1.0` and `alpha = 0.5`).
/// See the `compositing2_matches_cpp_reference_*` regression tests.
