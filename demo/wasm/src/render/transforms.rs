...
use agg_rust::bounding_rect::{bounding_rect, bounding_rect_single};
...
/// Simple 3x3 box blur on the lion — matching C++ simple_blur.cpp.
///
/// params[0] = blur center x (default 100)
/// params[1] = blur center y (default 102)
pub fn simple_blur(width: u32, height: u32, params: &[f64]) -> Vec<u8> {
    let blur_cx = params.first().copied().unwrap_or(100.0);
    let blur_cy = params.get(1).copied().unwrap_or(102.0);

    let (mut path, colors, path_idx) = crate::lion_data::parse_lion();
    let npaths = colors.len();
    let path_ids: Vec<u32> = path_idx.iter().map(|&i| i as u32).collect();
    let bbox = bounding_rect(&mut path, &path_ids, 0, npaths)
        .unwrap_or(agg_rust::basics::RectD::new(0.0, 0.0, 240.0, 380.0));
    let base_dx = (bbox.x2 - bbox.x1) * 0.5;
    let base_dy = (bbox.y2 - bbox.y1) * 0.5;
    ...
    // Left side: filled lion.
    let mut mtx = TransAffine::new();
    mtx.multiply(&TransAffine::new_translation(-base_dx, -base_dy));
    mtx.multiply(&TransAffine::new_scaling_uniform(1.0));
    mtx.multiply(&TransAffine::new_rotation(std::f64::consts::PI));
    mtx.multiply(&TransAffine::new_skewing(0.0, 0.0));
    mtx.multiply(&TransAffine::new_translation(
        width as f64 / 4.0,
        height as f64 / 2.0,
    ));
    ...
    // Right side: outline lion (translated by +width/2), matching C++ outline pass.
    let mut mtx_right = mtx;
    mtx_right.multiply(&TransAffine::new_translation(width as f64 / 2.0, 0.0));
    ...
    for i in 0..npaths {
        let start = path_idx[i] as u32;
        let mut transformed = ConvTransform::new(&mut path, mtx_right);
        ren_oaa.set_color(colors[i]);
        ras_oaa.add_path(&mut transformed, start, &mut ren_oaa);
    }
    drop(ren_oaa);

    // Two concentric strokes around the blur area.
    let mut ell = Ellipse::new(blur_cx, blur_cy, 100.0, 100.0, 100, false);
    let mut ell_stroke1 = ConvStroke::new(&mut ell);
    ell_stroke1.set_width(6.0);
    let mut ell_stroke2 = ConvStroke::new(&mut ell_stroke1);
    ell_stroke2.set_width(2.0);
    ras.reset();
    ras.add_path(&mut ell_stroke2, 0);
    render_scanlines_aa_solid(&mut ras, &mut sl, &mut rb, &Rgba8::new(0, 51, 0, 255));

    // Copy current output and apply simple blur only inside the ellipse.
    apply_simple_blur_ellipse(&mut buf, width, height, blur_cx, blur_cy, 100.0, 100.0);

    buf
}

fn sample_simple_blur_rgb24(src_rgb: &[u8], width: u32, height: u32, x: i32, y: i32) -> (u8, u8, u8) {
    ...
    if x > 0 && x < (width as i32 - 1) {
        ...
        for yy in (y - 1)..=(y + 1) {
            for xx in (x - 1)..=(x + 1) {
                let i = (yy as usize * stride) + (xx as usize * 3);
                r += src_rgb[i] as u32;
                g += src_rgb[i + 1] as u32;
                b += src_rgb[i + 2] as u32;
            }
        }
        return ((r / 9) as u8, (g / 9) as u8, (b / 9) as u8);
    }

    (0, 0, 0)
}

fn apply_simple_blur_ellipse(
    buf: &mut [u8],
    width: u32,
    height: u32,
    cx: f64,
    ...
    // Match C++ simple_blur.cpp semantics: blur reads from an opaque rgb24 source.
    // Any RGBA pixels are first composited over white so alpha cannot skew blur colors.
    let mut src_rgb = vec![0u8; (width * height * 3) as usize];
    for i in 0..(width as usize * height as usize) {
        let si = i * 4;
        let di = i * 3;
        let a = buf[si + 3] as u32;
        src_rgb[di] = ((buf[si] as u32 * a + 255 * (255 - a) + 127) / 255) as u8;
        src_rgb[di + 1] = ((buf[si + 1] as u32 * a + 255 * (255 - a) + 127) / 255) as u8;
        src_rgb[di + 2] = ((buf[si + 2] as u32 * a + 255 * (255 - a) + 127) / 255) as u8;
    }
    let mut ras = RasterizerScanlineAa::new();
    let mut sl = ScanlineU8::new();
    ...
    while ras.sweep_scanline(&mut sl) {
        ...
        for span in spans {
            ...
            for i in 0..len {
                ...
                let (sr, sg, sb) = sample_simple_blur_rgb24(&src_rgb, width, height, x, y);
                let cov = covers[span.cover_offset + i] as u32;
                let di = ((y as usize * width as usize + x as usize) * 4) as usize;
                let dr = buf[di] as u32;
                let dg = buf[di + 1] as u32;
                let db = buf[di + 2] as u32;
                buf[di] = ((dr * (255 - cov) + sr as u32 * cov + 127) / 255) as u8;
                buf[di + 1] = ((dg * (255 - cov) + sg as u32 * cov + 127) / 255) as u8;
                buf[di + 2] = ((db * (255 - cov) + sb as u32 * cov + 127) / 255) as u8;
                buf[di + 3] = 255;
            }
        }
    }
...
fn blur_subrect(
    ...
    match method {
        ...
        _ => {
            let rr = radius.round().clamp(0.0, 254.0) as u32;
            // AGG blur.cpp channels mode uses stack blur on individual channels.
            agg_rust::blur::stack_blur_rgba32(&mut sub_ra, rr, rr);
...
pub fn blur_demo(width: u32, height: u32, params: &[f64]) -> Vec<u8> {
    ...
    // Match C++: bounds are computed on the conv_curve view of the shape.
    // Never bail out to white output; keep deterministic defaults even if bounds fail.
    let (sx1, sy1, sx2, sy2) = {
        let mut shape_curve_for_bounds = ConvCurve::new(&mut shape);
        bounding_rect_single(&mut shape_curve_for_bounds, 0)
            .map(|r| (r.x1, r.y1, r.x2, r.y2))
            .unwrap_or((150.0, 90.0, 330.0, 275.0))
    };
    ...
    // Match agg-web blur.cpp shadow source color.
    {
        let mut shadow_curve = ConvCurve::new(&mut shadow);
        let mut shadow_trans = ConvTransform::new(&mut shadow_curve, shadow_persp);
        ras.reset();
        ras.add_path(&mut shadow_trans, 0);
        render_scanlines_aa_solid(&mut ras, &mut sl, &mut rb, &Rgba8::new(51, 77, 0, 255));
    }

    let mut blur_ms = 0.0;
    // Match C++: bbox is measured on transformed curve geometry.
    let mut shadow_curve_for_bounds = ConvCurve::new(&mut shadow);
    let mut shadow_trans_for_bounds = ConvTransform::new(&mut shadow_curve_for_bounds, shadow_persp);
    if let Some(bbox) = bounding_rect_single(&mut shadow_trans_for_bounds, 0) {
        let (mut bx1, mut by1, mut bx2, mut by2) = (bbox.x1, bbox.y1, bbox.x2, bbox.y2);
        bx1 -= radius;
        by1 -= radius;
        bx2 += radius;
        by2 += radius;
        if method == 1 {
            // Extra right/top expansion is only for recursive blur in agg-web blur.cpp.
            bx2 += radius;
            by2 += radius;
        }
        ...
        if method != 2 || channel_r || channel_g || channel_b {
            blur_subrect(
                &mut buf,
                width,
                height,
...
/// Port of C++ `gouraud_mesh.cpp`.
///
/// params[0] = animation step (integer, default 0)
/// params[1] = override count N (default 0)
/// params[2..] = N triples: [vertex_idx, x, y]
pub fn gouraud_mesh(width: u32, height: u32, params: &[f64]) -> Vec<u8> {
    let mesh = build_gouraud_mesh_state(width, height, params);
    ...
    for edge in &mesh.edges {
        ...
        rasc.move_to_d(mesh.vertices[edge.p1].x, mesh.vertices[edge.p1].y);
        rasc.line_to_d(mesh.vertices[edge.p2].x, mesh.vertices[edge.p2].y);
    }
    render_gouraud_compound(&mut rasc, &mut rb, &mesh.vertices, &mesh.triangles);
    #[cfg(target_arch = "wasm32")]
    let tm_ms = js_sys::Date::now() - t0;
    #[cfg(not(target_arch = "wasm32"))]
    let tm_ms = t0.elapsed().as_secs_f64() * 1000.0;

    let mut ras = RasterizerScanlineAa::new();
    let mut sl = ScanlineU8::new();
    let mut txt = GsvText::new();
    txt.size(10.0, 0.0);
    txt.start_point(10.0, 10.0);
    txt.text(&format!(
        "{:.2} ms, {} triangles, {:.0} tri/sec",
        tm_ms,
        mesh.triangles.len(),
        if tm_ms > 0.0 {
            mesh.triangles.len() as f64 / tm_ms * 1000.0
        } else {
            0.0
        }
    ));
...
pub fn gouraud_mesh_pick_vertex(
    width: u32,
    height: u32,
    params: &[f64],
    x: f64,
    ...
    for (i, p) in mesh.vertices.iter().enumerate() {
        let d = ((p.x - x) * (p.x - x) + (p.y - y) * (p.y - y)).sqrt();
        if d < best_dist {
            best_dist = d;
            best_idx = i as i32;
        }
...
struct GouraudMeshState {
    vertices: Vec<MeshPoint>,
    triangles: Vec<MeshTriangle>,
    edges: Vec<MeshEdge>,
}

fn mesh_lcg_next(state: &mut u32) -> u32 {
    *state = state.wrapping_mul(1103515245).wrapping_add(12345);
    *state
}

fn mesh_rand_u8(state: &mut u32) -> u8 {
    ((mesh_lcg_next(state) >> 16) & 0xFF) as u8
}

fn mesh_rand_bool(state: &mut u32) -> u8 {
    (mesh_lcg_next(state) & 1) as u8
}

fn mesh_rand_range(state: &mut u32, v1: f64, v2: f64) -> f64 {
    let n = (mesh_lcg_next(state) % 1000) as f64;
    (v2 - v1) * n / 999.0 + v1
}

fn build_gouraud_mesh_state(width: u32, height: u32, params: &[f64]) -> GouraudMeshState {
    let cols = 20usize;
    let rows = 20usize;
    let cell_w = 17.0f64;
    let cell_h = 17.0f64;
    ...
    for i in 0..rows {
        for j in 0..cols {
            let x = start_x + j as f64 * cell_w;
            let y = start_y + i as f64 * cell_h;
            vertices.push(MeshPoint {
                x,
                y,
                dx: mesh_rand_range(&mut rng, -0.5, 0.5),
                dy: mesh_rand_range(&mut rng, -0.5, 0.5),
                color: Rgba8::new(
                    mesh_rand_u8(&mut rng) as u32,
                    mesh_rand_u8(&mut rng) as u32,
                    mesh_rand_u8(&mut rng) as u32,
                    255,
                ),
                dc_r: mesh_rand_bool(&mut rng),
                dc_g: mesh_rand_bool(&mut rng),
                dc_b: mesh_rand_bool(&mut rng),
            });
        }
    }

    let mut triangles = Vec::with_capacity((rows - 1) * (cols - 1) * 2);
    let mut edges = Vec::with_capacity((rows - 1) * (cols - 1) * 3 + rows + cols);
    for i in 0..(rows - 1) {
        for j in 0..(cols - 1) {
            let p1 = i * cols + j;
            let p2 = p1 + 1;
            ...
            let curr_cell = (i * (cols - 1) + j) as i32;
            let left_cell = if j > 0 { curr_cell - 1 } else { -1 };
            let bott_cell = if i > 0 {
                curr_cell - (cols - 1) as i32
            } else {
                -1
            };

            let curr_t1 = curr_cell * 2;
            let curr_t2 = curr_t1 + 1;
            let left_t1 = if left_cell >= 0 { left_cell * 2 } else { -1 };
            let bott_t2 = if bott_cell >= 0 { bott_cell * 2 + 1 } else { -1 };
    ...
    let anim_step = params.first().copied().unwrap_or(0.0).max(0.0) as usize % 256;
    for _ in 0..anim_step {
        mesh_randomize_points(
            &mut vertices,
            cols,
            rows,
            cell_w,
        ...
        mesh_rotate_colors(&mut vertices);
    }

    let mut at = 2usize;
    let override_count = params.get(1).copied().unwrap_or(0.0).max(0.0) as usize;
    for _ in 0..override_count {
        if at + 2 >= params.len() {
            break;
        }
        let idx = params[at].max(0.0) as usize;
        let x = params[at + 1];
        let y = params[at + 2];
        if idx < vertices.len() {
            vertices[idx].x = x.clamp(0.0, width as f64);
            vertices[idx].y = y.clamp(0.0, height as f64);
        }
        at += 3;
    }
...
fn mesh_randomize_points(
    vertices: &mut [MeshPoint],
    cols: usize,
    rows: usize,
    cell_w: f64,
    cell_h: f64,
    ...
    for i in 0..rows {
        for j in 0..cols {
            let idx = i * cols + j;
            let xc = j as f64 * cell_w + start_x;
            let yc = i as f64 * cell_h + start_y;
            let x1 = xc - cell_w / 4.0;
            let y1 = yc - cell_h / 4.0;
            let x2 = xc + cell_w / 4.0;
            let y2 = yc + cell_h / 4.0;
            let p = &mut vertices[idx];
            p.x += p.dx;
            p.y += p.dy;
            if p.x < x1 {
                p.x = x1;
...
fn mesh_rotate_colors(vertices: &mut [MeshPoint]) {
    for p in vertices.iter_mut().skip(1) {
        let mut r = p.color.r as i32 + if p.dc_r != 0 { 5 } else { -5 };
        let mut g = p.color.g as i32 + if p.dc_g != 0 { 5 } else { -5 };
        let mut b = p.color.b as i32 + if p.dc_b != 0 { 5 } else { -5 };
        if r < 0 {
            r = 0;
            p.dc_r ^= 1;
        }
...
fn render_gouraud_compound(
    ...
    for tri in triangles {
        let p1 = &vertices[tri.p1];
        let p2 = &vertices[tri.p2];
        let p3 = &vertices[tri.p3];
    ...
    loop {
        ...
        for s in 0..num_styles {
            ...
            if rasc.sweep_scanline(&mut sl, s as i32) && style_id < gouraud_spans.len() {
                let y = Scanline::y(&sl);
                for span in sl.begin() {
                    ...
                    if len > 0 {
                        ...
                        gouraud_spans[style_id].generate(&mut colors, x, y, len as u32);
                        let covers =
...
mod tests {
    ...
    #[test]
    #[ignore = "requires local C++ renderer binary"]
    fn blur_matches_cpp_reference_stack_mode() {
        let width = 440u32;
        let height = 330u32;
        let params = [
            15.0, // radius
        ...
        let rust = blur_demo(width, height, &params);
        ...
        let out_path: PathBuf = std::env::temp_dir().join("agg_blur_cpp.raw");
        if out_path.exists() {
            fs::remove_file(&out_path).expect("failed to remove old temp raw output");
        }
        ...
        assert_eq!(
            different_pixels,
            0,
            "blur mismatch: {} differing pixels, max_channel_diff={}, first_diff={:?}",
            different_pixels,
            max_channel_diff,
            first_diff
        );
